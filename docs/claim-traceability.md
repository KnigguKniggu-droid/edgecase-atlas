...
| The hosted application "does not expose subprocesses, arbitrary HTTP, uploads, or user code execution." | README.md:16 (original wording) | Subprocess, arbitrary HTTP, and code execution are correctly excluded (`app/ui.py:104-108`). **Uploads are exposed**: `app/app_pages/compare_runs.py:89-101` and `:146-151` render three `st.file_uploader` controls; `.streamlit/config.toml` sets `maxUploadSize = 2`; `app/artifact_io.py:126` `ingest_artifact` is the documented upload entry point | FALSE (corrected) |
